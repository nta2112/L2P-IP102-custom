...
# ==============================================================================
"""A config for continual learning on IP102 (open-world lifelong retrieval).

25 classes, 4 tasks (7/6/6/6), PASS class order (seed 1993). The dataset is
auto-discovered (env IP102_DATA_ROOT, /kaggle/input, or a local folder), see
``libml/ip102_data.py``.
"""
...
def get_config():
  ...
  # IP102 images are resized like ImageNet inputs.
  ...
  config.continual.num_tasks = 4
  config.continual.num_classes_per_task = 7
  config.continual.rand_seed = 1993
  ...
  # memory_size caps the number of training images per class
  # (0 = use all training images). Cap per class = memory_size // num_seen.
  config.continual.memory_size = 0
